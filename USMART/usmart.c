...
#define USMART_REC_LEN 200

u16 USMART_RX_STA=0;
u8 USMART_RX_BUF[USMART_REC_LEN];
u8 aRxBuffer = 0;
...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{					   
		...
		case 1://帮助指令
			printf("\r\n");
		//汉字报错 在C/C++ Misc Control输入 --no-multibyte-chars
#if USMART_USE_HELP
			printf("------------------------USMART V3.1------------------------ \n");
			printf("USMART是由ALIENTEK开发的一个灵巧的串口调试互交组，通过\n");
...
void usmart_usartit(void)
{
		if((USMART_RX_STA&0x8000)==0)
		{
			if(USMART_RX_STA&0x4000)
			{
				if(aRxBuffer!=0x0a)USMART_RX_STA=0;
				else USMART_RX_STA|=0x8000;
			}
			else //
			{	
				if(aRxBuffer==0x0d)USMART_RX_STA|=0x4000;
				else
				{
					USMART_RX_BUF[USMART_RX_STA&0X3FFF]=aRxBuffer;
					USMART_RX_STA++;
					if(USMART_RX_STA>(USMART_REC_LEN-1))USMART_RX_STA=0;//  
				}
			}
